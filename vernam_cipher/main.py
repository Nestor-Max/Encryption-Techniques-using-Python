...
class VernamCipherForm(FlaskForm):
...
def home():
    ...
    if form.validate_on_submit():
        ...
        mode = form.mode.data
        if mode == 'encrypt':
            result = vernam_cipher(text, key, encrypt=True)
        else:
            result = vernam_cipher(text, key, encrypt=False)
        file_path = os.path.join(app.config['UPLOAD_FOLDER'], 'vernam_output.txt')
        write_file(file_path, result)
    return render_template('vernam.html', form=form, result=result)
